...
export class VoteComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.subscriptions.push(
        this.aService.selected.asObservable().subscribe((selected: any) => {
          ...
          if (selected && selected['name'] && this.selectedAccountName !==
            ...
            this.unstakeTime = moment.utc(selected.unstakeTime).
                add(72, 'hours').
                fromNow();
  ...
  edAutoClaim(check?) {
    ...
    this.verifyAutoClaimSetup(this.aService.selected.getValue()).
        catch(console.log);
  }
  ...
  async createClaimPermission() {
    ...
    const subs = this.trxFactory.status.subscribe((event) => {
      if (event === 'done') {
        ...
        this.autoClaimStatus = true;
        subs.unsubscribe();
        this.checkWaxGBMdata(this.aService.selected.getValue().name).
            then(() => {
              if (this.claimReady) {
                this.claimDirect(false).catch(console.log);
              }
  ...
  private async checkWaxGBMdata(name: any) {
    ...
    if (data) {
      ...
      this.last_claim_time = moment(moment.utc(this.gbmLastClaim).toDate()).
          toDate().
          getTime();
      this.gbmLastClaim = moment(moment.utc(this.gbmLastClaim).toDate()).
          local().
      ...
      this.gbmNextClaim = moment.utc(this.last_claim_time).
          add(1, 'day').
          fromNow();
