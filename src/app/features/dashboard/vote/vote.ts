    ...
    <div class="vote-view">
      ...
      <div class="summary-row">
        <div class="summary-card">
          ...
          <span class="summary-value">{{
            wallet.selectedAccount()!.info.core_liquid_balance ?? '0.0000'
          }}</span>
        </div>
        <div class="summary-card">
        ...
        <div class="summary-card">
          ...
          <span class="summary-value" [class.decay]="voteDecayPct() > 10">{{
            voteDecayDisplay()
          }}</span>
        </div>
      </div>
        ...
        <div class="section-card">
          ...
          @if (features.isFreeChain()) {
            <p class="section-desc">
              Staking is optional — it provides voting weight and transaction priority on
              {{ wallet.selectedAccount()!.chainName }}.
            </p>
          } @else {
            <p class="section-desc">
              Staked tokens are used for voting and resource allocation. You can unstake at any
              time.
            </p>
          }
      ...
      <div class="section-card">
        <div class="section-tabs">
          <button class="tab" [class.active]="voteTab() === 'bp'" (click)="voteTab.set('bp')">
            Vote for Block Producers
          </button>
          <button class="tab" [class.active]="voteTab() === 'proxy'" (click)="voteTab.set('proxy')">
        ...
        @if (voteTab() === 'bp') {
          <p class="section-desc">
            Select up to 30 block producers to vote for. Your vote weight is based on your staked
            tokens.
          </p>
          ...
          @if (loadingProducers()) {
            ...
            <div class="producers-table">
              ...
              @for (bp of filteredProducers(); track bp.owner; let i = $index) {
                <div
                  class="table-row"
                  [class.selected]="selectedProducers().includes(bp.owner)"
                  (click)="toggleProducer(bp.owner)"
                >
                  <span class="col-check">
                    <span
                      class="checkbox"
                      [class.checked]="selectedProducers().includes(bp.owner)"
                    ></span>
                  </span>
        ...
        @if (voteTab() === 'proxy') {
          <p class="section-desc">
            Choose a proxy to vote for block producers on your behalf. You can only vote through one
            proxy.
          </p>
          ...
          <div class="form-group">
            ...
            <input
              ...
              [value]="proxyAccount()"
              (input)="proxyAccount.set($any($event.target).value)"
            />
          </div>

          <button class="btn-primary" [disabled]="!proxyAccount()" (click)="onConfirmProxy()">
            CONFIRM VOTE
          </button>
        }
      </div>
...
export class VoteComponent {
  ...
  async onConfirmVote() {
    ...
    await this.confirmAction('Vote for Producers', [
      {
        account: this.wallet.systemAccount('voteproducer'),
        name: 'voteproducer',
        authorization: this.auth(),
        data,
      },
