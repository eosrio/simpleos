...
@Component({
  ...
  template: `
    <div class="rex-view">
      <h2>REX</h2>
      <p class="page-desc">
        Resource Exchange — stake tokens into REX to earn rewards from network fees and RAM trading.
      </p>
        ...
        <div class="section-card">
          ...
          <div class="form-group">
            ...
            <input
              ...
              placeholder="0.0000"
              [value]="buyAmount()"
              (input)="buyAmount.set($any($event.target).value)"
            />
          </div>
          <button class="btn-primary" [disabled]="!buyAmount()" (click)="onBuyRex()">
            BUY REX
          </button>
        </div>
        ...
        <div class="section-card">
          ...
          <div class="form-group">
            ...
            <input
              ...
              placeholder="0.0000"
              [value]="sellAmount()"
              (input)="sellAmount.set($any($event.target).value)"
            />
          </div>
          <button class="btn-primary" [disabled]="!sellAmount()" (click)="onSellRex()">
            SELL REX
          </button>
        </div>
      </div>
      ...
      <div class="section-card">
        ...
        <div
          class="rex-info-row"
          style="margin-top: var(--sp-4); border-top: 1px solid var(--border-subtle); padding-top: var(--sp-4);"
        >
          <div class="rex-info">
...
export class RexComponent {
        ...
        if (maturities.length === 0) {
          ...
          if (nextMaturity <= now) {
            ...
            this.rexMaturityLabel.set(
              hoursLeft > 24 ? `${Math.ceil(hoursLeft / 24)}d left` : `${hoursLeft}h left`,
            );
          ...
          for (const mat of maturities) {
            ...
            if (matTime && matVal) {
              const maturityTimeHours =
                (new Date(matTime + 'Z').getTime() - Date.now()) / (1000 * 60 * 60);
  ...
  async onBuyRex() {
    ...
    await this.confirmAction('Buy REX', [
      {
        account: this.wallet.systemAccount('deposit'),
        name: 'deposit',
        authorization: this.auth(),
        data: { owner: this.me(), amount: this.qty(amt) },
      },
      {
        account: this.wallet.systemAccount('buyrex'),
        name: 'buyrex',
        authorization: this.auth(),
        data: { from: this.me(), amount: this.qty(amt) },
      },
    ]);
    this.buyAmount.set('');
  }
  ...
  async onSellRex() {
    ...
    await this.confirmAction('Sell REX', [
      {
        account: this.wallet.systemAccount('sellrex'),
        name: 'sellrex',
        authorization: this.auth(),
        data: { from: this.me(), rex: this.qty(amt) },
      },
      {
        account: this.wallet.systemAccount('withdraw'),
        name: 'withdraw',
        authorization: this.auth(),
        data: { owner: this.me(), amount: this.qty(amt) },
      },
