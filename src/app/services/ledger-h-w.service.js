...
var LedgerHWService = /** @class */ (function () {
    function LedgerHWService(eosService) {
        ...
        this.ledgerStatus = new rxjs_1.BehaviorSubject(false);
    ...
    LedgerHWService_1 = LedgerHWService;
    LedgerHWService.convert = function (response) {
        return {
    ...
    LedgerHWService.prototype.encode = function (writter, type, data) {
        writter['writeBuffer'](this.fcbuffer.toBuffer(type, data), this.asn1['Ber']['OctetString']);
    };
    LedgerHWService.prototype.serialize = function (transaction) {
        var _this = this;
    ...
    LedgerHWService.prototype.isElectron = function () {
        this.electronStatus = window && window['process'] && window['process']['type'];
        return this.electronStatus;
    };
    LedgerHWService.prototype.readNextPublicKey = function (index, transport, limit, resolve) {
        var _this = this;
        ...
        transport.send(0xD4, 0x02, 0x00, 0x01, buffer).then(function (response) {
            ...
            if (_this.eosService.ecc.isValidPublic(result['wif'])) {
                ...
                _this.eosService.eos.getKeyAccounts(result['wif']).then(function (data) {
                    ...
                    if (index + 1 < limit) {
                        _this.readNextPublicKey(index + 1, transport, limit, resolve);
                    }
                    else {
                        resolve(_this.slots);
                    }
    ...
    LedgerHWService.prototype.submitToLedger = function (payload, index, transport, resolve) {
        var _this = this;
        ...
        transport.send(0xD4, 0x04, index === 0 ? 0x00 : 0x80, 0x00, payload[index]).then(function (output) {
            ...
            if (index < payload.length - 1) {
                ...
                var result = LedgerHWService_1.convert(output);
                var rawSig = result.v + result.r + result.s;
                resolve(rawSig);
            }
        });
    };
    LedgerHWService.prototype.enableLedgerEOS = function (slot) {
        var _this = this;
        console.log('Enabling ledger connection on slot ' + slot.toString());
        var newPath = this.basePath + slot.toString();
        var pSigner = function (data) {
            return new Promise(function (resolve) {
                var paths = _this.bippath.fromString(newPath).toPathArray();
    ...
    LedgerHWService.prototype.readPublicKeys = function (slots) {
        var _this = this;
        this.slots = [];
        return new Promise(function (resolve) {
            _this.readNextPublicKey(0, _this.activeTransport, slots, resolve);
        });
    };
    LedgerHWService.prototype.getAppConfig = function () {
    ...
    LedgerHWService.prototype.initListener = function () {
        var _this = this;
        this.ledgerTransport.listen({
            'next': function (event) {
                if (event.type === 'add') {
                    console.log('Adding device!');
                    _this.ledgerStatus.next(true);
                    ...
                    _this.ledgerStatus.next(false);
                }
            }
        });
    };
    var LedgerHWService_1;
    LedgerHWService = LedgerHWService_1 = __decorate([
        core_1.Injectable({
            providedIn: 'root'
        }),
        __metadata("design:paramtypes", [eosjs_service_1.EOSJSService])
    ], LedgerHWService);
    return LedgerHWService;
}());
exports.LedgerHWService = LedgerHWService;
//# sourceMappingURL=ledger-h-w.service.js.map