...
export class Eosjs2Service {
	...
	async createAccount(creator: string , name: string , owner: string ,
		...
		_actions.push ( {
			...
			name: 'newaccount' ,
			authorization: [ auth ] ,
			data: {creator: creator , name: name , owner: _onwer , active: _avtive}
		} );
		_actions.push ( {
			account: 'eosio' ,
			name: 'buyrambytes' ,
	...
	async changebw(account , permission , amount , symbol , ratio , fr) {
		...
		if ((typeof accountInfo[ 'cpu_weight' ]) === 'string') {
			wei_cpu = Math.round ( parseFloat ( accountInfo[ 'cpu_weight' ].split ( ' ' )[ 0 ] ) / _div );
			wei_net = Math.round ( parseFloat ( accountInfo[ 'net_weight' ].split ( ' ' )[ 0 ] ) / _div );
		} else {
		...
		if (refund) {
			ref_cpu = Math.round ( parseFloat ( refund[ 'cpu_amount' ].split ( ' ' )[ 0 ] ) * _div );
			ref_net = Math.round ( parseFloat ( refund[ 'net_amount' ].split ( ' ' )[ 0 ] ) * _div );
		}
