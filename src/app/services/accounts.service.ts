...
export class AccountsService {
  ...
  async getAccActions(account) {
    ...
    await this.getActions(account, 0, 12).catch(console.log);
  }
  ...
  async getActions(account: string, pos: number, offset: number, filter?, after?, before?, parent?) {

    this.actions = [];

    // check history using hyperion
    const hyperionStatus = await this.getActionsHyperionMulti(account, pos, offset, filter, after, before, parent);
    if (hyperionStatus) {
      return;
    }
