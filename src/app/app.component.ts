...
export class AppComponent implements OnInit, AfterViewInit {
    ...
    public resourceTransactionPayload: any;
    ...
    async resourceInit(authSelected?) {
        ...
        const response = await this.resource.checkResource(auth, this.action_json, undefined, undefined, this.aService.activeChain['symbol']);
        this.resourceInfo = {
    ...
    async signExternalAction() {
        ...
        if (this.mode === 'local') {
            ...
            try {
                ...
                const result = await this.eosjs.signTrx(transactionPayload, !this.transitMode);
                // console.log(result);
            ...
            try {
                ...
                if (this.resourceInfo['relay'] && this.useFreeTransaction === '1') {
                    ...
                    if (!result.ok) {
                        const e: RpcError = result.error.error;
                        this.signResponse({
                            status: 'ERROR',
                            reason: e
                        });
