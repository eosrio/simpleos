...
export class SimpleosConnectService {
    ...
    onGetAuthorizations(chainId, callback): void {
        ...
        ipcMain.once('changeChainResponse', (event, changeChainResp) => {
            if (changeChainResp) {
                ...
                ipcMain.once('authorizationsResponse', (event2, authorizations) => {
    ...
    onLogIn(sessionUuid, authorization, callback): void {
        Logger.info('onLogIn');
    ...
    onIsLoggedIn(sessionUuid, callback): void {
        Logger.info('isLoggedIn');
        callback(
            this.currentSessionUuid === sessionUuid && this.currentAuthorization);
    }

    onGetCurrentAuthorization(sessionUuid, callback): void {
    ...
    onTransact(transaction, callback): void {
